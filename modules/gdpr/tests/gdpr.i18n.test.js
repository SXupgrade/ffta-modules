...
test('gdpr i18n packs cover every app.t gdpr key', () => {
  const en = readJson('i18n/en.json');
  const fr = readJson('i18n/fr.json');
  const keys = collectTranslationKeys();

  assert.ok(keys.length > 0, 'Expected at least one gdpr i18n key usage.');
...
test('gdpr i18n packs use the same nested shape as other modules', () => {
  const en = readJson('i18n/en.json');
  const fr = readJson('i18n/fr.json');
