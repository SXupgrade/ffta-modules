...
function collectTranslationKeys() {
  ...
  while (stack.length) {
    ...
    for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
      ...
      const regex = /app\.t\(\s*['"](ruleBuilder\.[^'"]+)['"]/g;
      let match;
...
test('rule-builder i18n packs cover every app.t ruleBuilder key', () => {
  const en = readJson('i18n/en.json');
  const fr = readJson('i18n/fr.json');
  const keys = collectTranslationKeys();

  assert.ok(keys.length > 0, 'Expected at least one ruleBuilder i18n key usage.');
...
test('rule-builder i18n packs use the same nested shape as other modules', () => {
  const en = readJson('i18n/en.json');
  const fr = readJson('i18n/fr.json');
