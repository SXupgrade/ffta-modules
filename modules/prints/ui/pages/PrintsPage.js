export function mountPrintsPage({ root, app }) {
  const moduleInfo = app.services.get('prints.legacy');
...
function buildLegacyPage({ app, moduleInfo }) {
  const title = app.t('prints.title');
  const description = app.t('prints.description');
  const openLabel = app.t('app.actions.open');
  const iframeTitle = escapeHtml(title);
  return `
    <section class="ffta-page legacy-module-page legacy-module-page--prints">
