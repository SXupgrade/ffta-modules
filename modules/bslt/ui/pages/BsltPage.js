export function mountBsltPage({ root, app }) {
  const moduleInfo = app.services.get('bslt.legacy');
...
function buildLegacyPage({ app, moduleInfo }) {
  const title = app.t('bslt.title');
  const description = app.t('bslt.description');
  const openLabel = app.t('app.actions.open');
  const iframeTitle = escapeHtml(title);
  return `
    <section class="ffta-page legacy-module-page legacy-module-page--bslt">
