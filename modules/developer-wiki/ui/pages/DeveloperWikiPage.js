...
export function mountDeveloperWikiPage({ root, app }) {
  ...
  function render() {
    root.innerHTML = buildHtml({ query, activeSectionId }, app);
    // Self-correct after render: if the current selection was filtered out by
    // the search query, fall back to the first still-visible section so the
    // next click/search interaction starts from a valid state.
    const filteredSections = getFilteredSections(query);
  ...
  function handleClick(event) {
    ...
    if (copyButton) {
      ...
      navigator.clipboard?.writeText(code).then(() => {
        app.notify.success(app.t('developer-wiki.page.copied'));
      }).catch(() => {
...
export function buildHtml({ query, activeSectionId }, app) {
  const filteredSections = getFilteredSections(query);
  const activeSection = wikiSections.find((section) => section.id === activeSectionId && filteredSections.some((visible) => visible.id === activeSectionId))
    || filteredSections[0]
    || wikiSections[0];
    ...
    <section class="ffta-page developer-wiki-page">
      <div class="ffta-page__header developer-wiki-hero">
        <div>
          ...
          <h1>${escapeHtml(app.t('developer-wiki.page.title'))}</h1>
          <p class="ffta-muted">${escapeHtml(app.t('developer-wiki.page.description'))}</p>
        </div>
        <div class="developer-wiki-callout">${escapeHtml(app.t('developer-wiki.page.labFirst'))}</div>
      </div>

      <div class="developer-wiki-layout">
        <aside class="developer-wiki-sidebar">
          <input class="developer-wiki-search" data-action="search" value="${escapeAttribute(query)}" placeholder="${escapeAttribute(app.t('developer-wiki.page.searchPlaceholder'))}">
          <nav class="developer-wiki-nav" aria-label="Developer wiki sections">
            ${filteredSections.map((section) => buildNavItem(section, activeSection?.id, app)).join('') || buildEmptySearch(app)}
          </nav>
        </aside>
...
function buildNavItem(section, activeSectionId, app) {
